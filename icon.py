...
def icon_detection_cycle(selected_poi, is_create_custom_poi):
    ...
    if is_create_custom_poi:
        if center_mass_screen:
            ...
            print("Cannot create custom POI: Player icon not located.")
            speaker.speak("Cannot create custom POI: Player icon not located.")
        return
...
def handle_poi_selection(selected_poi, center_mass_screen):
    ...
    if poi_name == 'safe zone':
        ...
        if center_mass_screen:
            ...
            print("Cannot find closest POI: Player icon not located")
    ...
    if coordinates and len(coordinates) == 2:
        ...
        try:
            ...
            if center_mass_screen:
                ...
                print(f"Clicked on {poi_name} at ({x}, {y}), but can't process screenshot: Player icon not located")
                speaker.speak(f"Clicked on {poi_name}")
        except ValueError:
