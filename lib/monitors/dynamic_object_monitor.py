"""
Background dynamic object detection and spatial audio system
"""
import threading
import time
import os
...
class DynamicObjectAudioThread:
    """Manages audio for a single dynamic object with configurable ping intervals"""
    
    def __init__(self, dynamic_object_name: str, audio_instance: SpatialAudio, ping_interval: float):
        self.dynamic_object_name = dynamic_object_name
...
class DynamicObjectMonitor:
    ...
    def __init__(self):
        ...
        self.active_audio_threads = {}  # dynamic_object_name -> DynamicObjectAudioThread
    ...
    def initialize_audio(self):
        ...
        for dynamic_object_name in DYNAMIC_OBJECT_CONFIGS.keys():
            ...
            if os.path.exists(dynamic_object_sound_path):
                try:
                    audio_instance = SpatialAudio(dynamic_object_sound_path)
                    # Set up volume management for individual object sounds
                    config = read_config()
                    master_volume, individual_volume = SpatialAudio.get_volume_from_config(
                        config, f"{dynamic_object_name.replace('_', '').title()}Volume", 'MasterVolume', 1.0
                    )
                    audio_instance.set_master_volume(master_volume)
                    audio_instance.set_individual_volume(individual_volume)
                    self.audio_instances[dynamic_object_name] = audio_instance
                except Exception:
                    continue
    
    def get_audio_instance(self, dynamic_object_name: str) -> Optional[SpatialAudio]:
        """Get audio instance for specific object, fall back to default"""
        return self.audio_instances.get(dynamic_object_name, self.default_audio)
    
    def is_enabled(self) -> bool:
        """Check if dynamic object monitoring is enabled in config"""
        config = read_config()
        return get_config_boolean(config, 'MonitorDynamicObjects', False)
    
    def should_monitor(self) -> bool:
        """Check if monitoring should be active"""
        return self.is_enabled() and not monitor.map_open
    
    def is_object_enabled(self, dynamic_object_name: str) -> bool:
        """Check if a specific object type is enabled for monitoring"""
        config = read_config()
        config_key = f"Monitor{dynamic_object_name.replace('_', '').title()}"
        return get_config_boolean(config, config_key, True)
    
    def get_object_ping_interval(self, dynamic_object_name: str) -> float:
        """Get the ping interval for a specific object type"""
        config = read_config()
        config_key = f"{dynamic_object_name.replace('_', '').title()}PingInterval"
    ...
    def detect_objects_on_minimap(self) -> Dict[str, Tuple[int, int]]:
        ...
        try:
            enabled_objects = [
                obj_name for obj_name in DYNAMIC_OBJECT_CONFIGS.keys()
            ...
            detected_objects = optimized_finder.find_dynamic_objects_on_minimap_screen(enabled_objects)
            ...
            if self.should_play_audio_for_distance(distance):
                if obj_name in self.active_audio_threads:
                    ...
                    if audio_instance:
                        ...
                        audio_thread = DynamicObjectAudioThread(obj_name, audio_instance, ping_interval)
...
dynamic_object_monitor = DynamicObjectMonitor()
